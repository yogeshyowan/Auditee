...
-   **Operator Access Allowlist**: `artifacts/api-server/src/lib/accessOverrides.ts`
-   **ASPICE Video Walkthrough**: `artifacts/auditee-tutorial/src/components/video/aspice/` (`AspiceTour.tsx` + `AspiceScenes.tsx`, served at `?aspice=1`)
...
-   **GitHub merge gate (compliance-as-code)**: `routes/githubWebhook.ts` handles `push` and `pull_request.{opened,synchronize,reopened}` and POSTs a Check Run named "Auditee / Compliance" back to the head SHA. Verdict (`computeGateVerdict` in same file): any open `critical`/`high` CAPA OR any failed pipeline run in the last 24h on the **same branch or commit SHA** → `conclusion: "failure"`; else `success`. Pipeline-run filter is intentionally branch/SHA-scoped so an unrelated branch's failure can't fail this PR. Auth: per-source `cfg.token` else `GITHUB_PAT` (token needs `checks:write` for fine-grained, or `repo` for classic). Per-source opt-out: set `cfg.gateChecks = false`. When the repo's branch-protection rule requires this check, the GitHub merge button is literally disabled on a PR with regressions. Multi-project repos: only one check is posted per webhook (deterministic by lowest `source.id`) because the check-name is fixed and would race; if the same repo is tracked by multiple projects, only the lowest-`id` source's project drives the gate (logged as a warn). HTTP call goes through `lib/github-check-runs.ts → postCheckRun` using `safeFetch` for SSRF re-validation.
-   **Connector write-back (work items + documents)**: `lib/connector-push.ts` exposes 4 push helpers — `pushJiraIssue`, `pushAdoWorkItem`, `pushConfluencePage`, `pushSharepointDocument`. Re-uses the SAME source `config` shape as the matching read-side connector (Atlassian: `host/email/token`, ADO: `host/project/token`); SharePoint is push-only and uses Azure AD app-only auth (`tenantId/clientId/clientSecret/siteId`, requires `Sites.ReadWrite.All` application permission with admin consent). Surfaces: `routes/connectorPush.ts` exposes `GET /connector-push/targets?projectId=&type=workitem|document` (lists eligible connected sources), `POST /connector-push/work-item` (Jira/ADO), `POST /connector-push/document` (Confluence/SharePoint). Frontend: `components/PushToConnectorButton.tsx` is reused on Capa rows ("Create issue") and on every report row + viewer ("Publish to…"). Confluence body is generated via a tiny in-house `markdownToConfluenceStorage()` (no new deps); SharePoint receives raw `.md` content via Graph PUT to `/sites/{siteId}/drive/root:/{folderPath}/{title}.md:/content`. `confluence` and `sharepoint` are added to `SUPPORTED_KINDS` with an `isDocumentTargetKind` short-circuit on Sync (push-only, like pipelines). `webhookSecret`/`clientSecret`/`token` are all masked by `safeConfig`.
-   **Pipeline runs as 5th audit input**: Pipeline tools (CI/CD, CD, test runners, ETL/data, MLOps, security scanners, IaC) ingest **push-side** via `routes/pipelineWebhooks.ts`. ~37 tools front-end-listed in `Sources.tsx → PIPELINE_KIND_DEFS`, normalized into 6 backend adapters in `lib/pipeline-registry.ts` (`github_actions`, `gitlab_ci`, `jenkins`, `sarif_upload`, `junit_upload`, `generic_webhook`). Per-source webhook URL: `${origin}/api/integrations/pipelines/{kind}/webhook?source={id}`; per-source secret rotated via `POST /api/sources/:id/pipeline-secret` (HMAC-sha256 signature header OR shared bearer token, hex/base64 both accepted). Persisted to `pipeline_runs` + `pipeline_findings` (lib/db schema, both indexed by `(source_id, completed_at desc)`). The `compliance-audit` route in `routes/ai.ts` loads the 200 most recent runs and appends a "PIPELINE EVIDENCE" block (totals by category + 30 most recent) to the user prompt — the system prompt counts this as the 5th evidence input alongside code/requirements/defects/standards. Sync is a no-op for pipeline kinds (already inbound-only via `isPipelineKind` short-circuit in `routes/sources.ts`). The frontend `Kind` type in `Sources.tsx` is widened to `string` because the OpenAPI spec's strict union lags behind the connector catalog; `SUPPORTED_KINDS` in `routes/sources.ts` is the authoritative server-side allowlist.
-   **Operator access override**: `lib/accessOverrides.ts` holds a small allowlist (`DEFAULT_UNLIMITED_EMAILS` + `UNLIMITED_CREDIT_EMAILS` env, comma/whitespace separated). Listed emails are forced in-memory to `plan='enterprise'`, `seatLimit=PLAN_SEATS.enterprise`, `role='owner'` and `planExpiresAt=null` inside `requireWorkspace`/`optionalWorkspace` (after `expirePastDueAnnualPlan`), and are exempt from the per-tenant IP allowlist so they can't self-lockout. The credit middleware uses the same `isUnlimitedEmail` for unlimited AI credits. The DB row is never mutated — removing an email reverts that account on the next request. Default allowlist contains `yogesh.yowan@gmail.com` (project owner / test account).
-   **Tutorial background music**: `artifacts/auditee-tutorial/src/components/video/BackgroundMusic.tsx` is a no-op stub returning `null` — original Web Audio synth was removed but the component is kept so the 20+ existing imports still compile. Voiceover narration (HD via `/api/tutorial/tts`, fallback to browser SpeechSynthesis) is the only audio.
-   **ASPICE walkthrough video**: `artifacts/auditee-tutorial` exposes a long-form Automotive SPICE 4.0 tour at `?aspice=1`. `AspiceTour.tsx` chains 13 modules in user-narrative order (sources → interview → requirements → gaps → defects → compliance → traceability → capa → workflows → pdlc → reports → recurring-audits → analytics) with ASPICE chapter titles (ENG.1/ENG.2/ENG.5/SUP.7/SUP.9/SUP.10/MAN.3/MAN.6/SWE.4 mapped per ModuleKey in `ASPICE_FOR_MODULE`), plus a custom `EnterpriseScene` (SSO/RBAC/audit-log/SIEM cards) — total ~7 minutes. Each module brings its own `TimedCaptions` narration; the tour just sequences them. Linked from the marketing site at `eltegra-site/src/pages/DemoVideos.tsx` as the primary CTA.
