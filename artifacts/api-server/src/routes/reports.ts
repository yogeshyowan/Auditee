...
router.post("/reports/generate", asyncH(async (req, res) => {
  ...
  const sysPrompt = `You are Montana, an AI-native PDLC platform that produces enterprise-grade ${kind.replace(
    "_",
    " ",
  )} reports.
...
- Output JSON only.`;
  ...
  await db.insert(activityEventsTable).values({
    id: randomUUID(),
    kind: "report",
    message: `Montana generated ${kind.replace("_", " ")} report: ${content.title}`,
    actor: "Montana",
    entityCode: saved.id,
  });
...
router.post("/reports/:id/refine", asyncH(async (req, res) => {
  ...
  const sysPrompt = `You are Montana refining an existing report. Apply the user's instruction PRECISELY without re-writing untouched sections.
...
<div class="meta">Generated by Montana · ${new Date(report.updatedAt).toISOString().split("T")[0]} · tone: ${escapeHtml(report.tone)}</div>
...
function buildDocx(report: typeof aiReportsTable.$inferSelect): Document {
  ...
  paragraphs.push(
    new Paragraph({
      children: [
        new TextRun({
          text: `Generated by Montana · ${new Date(report.updatedAt).toISOString().split("T")[0]} · tone: ${report.tone}`,
          italics: true,
          color: "64748B",
        }),
      ],
