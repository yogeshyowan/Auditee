  ...
  return (
    <motion.div className="absolute inset-0 flex flex-col items-center justify-center px-[5vw]"
      ...
      <motion.h2 className="text-[4vw] font-black text-center leading-tight mb-6"
        ...
        From requirement
        <br /><span style={{ color: 'var(--color-accent)' }}>to evidence — one click.</span>
      </motion.h2>
      <motion.p className="text-[1.4vw] text-white/60 text-center max-w-[50vw]"
        initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.8 }}>
        Every node in the graph is a live link. Click any requirement to see exactly what implements it.
      </motion.p>
    </motion.div>
  );
}
  ...
  return (
    <motion.div className="absolute inset-0 flex flex-col items-center justify-center"
      ...
      <motion.div className="text-center" initial={{ scale: 0.7, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ type: 'spring', damping: 14 }}>
        <div className="text-[7vw] font-black text-[var(--color-accent)]">91%</div>
        <div className="text-[2vw] font-semibold text-white/80 -mt-2">Traceability Coverage</div>
        <div className="text-[1.2vw] text-white/40 mt-3">Req → code → test → audit · fully linked</div>
      </motion.div>
    </motion.div>
  );
}
