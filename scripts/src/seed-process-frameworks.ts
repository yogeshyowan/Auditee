...
type SeedFramework = {
  ...
  status: string;
  score: number;
  controls: Array<{ code: string; title: string; description: string; owner?: string }>;
};
...
async function run() {
  for (const fw of FRAMEWORKS) {
    await pool.query(
      ...
      [fw.id, fw.code, fw.name, fw.category, fw.status, fw.score, fw.controls.length],
    );
    ...
    for (const c of fw.controls) {
      ...
      const res = await pool.query(
        ...
        [id, fw.id, c.code, c.title, c.description, "not_assessed", c.owner ?? "Unassigned", 0],
      );
      if (res.rowCount && res.rowCount > 0) inserted++;
    }
    console.log(`${fw.code}: framework upserted, ${inserted} new controls inserted (total ${fw.controls.length}).`);
  }
  console.log("Done.");
  await pool.end();
}
